...
try {
    ...
    expect(str_contains($adminPageSource, 'id="nexus-section-select"') && str_contains($adminPageSource, 'id="nexus-section-previous"') && str_contains($adminPageSource, 'id="nexus-section-next"') && str_contains($adminPageSource, 'sectionSelect.addEventListener'), 'Theme Studio provides explicit section selection and previous/next navigation');
    ...
    expect(str_contains($themeCssSource, '.nexus-agent.nexus-header-gradient .app-header') && str_contains($themeCssSource, '.nexus-agent.nexus-header-glass .app-header'), 'header treatments have distinct rendered styles');
    expect(str_contains($themeCssSource, '.nexus-agent.nexus-navigation-pill .sidebar-menu .nav-link.active') && str_contains($themeCssSource, '.nexus-agent.nexus-navigation-rail .sidebar-menu .nav-link.active') && str_contains($themeCssSource, '.nexus-agent.nexus-navigation-outline .sidebar-menu .nav-link.active'), 'active navigation treatments cover top-level and nested links');
    expect(str_contains($themeCssSource, '.nexus-agent.nexus-sidebar-compact .sidebar-menu .nav-icon') && str_contains($themeCssSource, '.nexus-runtime-preview-frame'), 'compact sidebar changes live labels and is rendered through the runtime preview');
    ...
    expect(str_contains($themeCssSource, '.nexus-studio-editor > .card-footer') && str_contains($themeCssSource, 'position: sticky;') && str_contains($themeCssSource, '.nexus-preview-card .btn-group'), 'mobile editor actions remain reachable while preview controls stay contained');
    expect(str_contains($themeCssSource, '.nexus-section-select { display: block;') && str_contains($themeCssSource, '.nexus-preview-surface-tabs { display: none;') && str_contains($themeCssSource, 'box-shadow: 0 -0.65rem 1.5rem'), 'phone layout uses compact pickers and a persistent touch-friendly draft action bar');
