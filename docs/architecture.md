...
The settings schema allow-lists brand and browser text, portal copy, nine six-digit colors, logo sizing/alignment, authentication-background presentation, navigation treatments, bounded sidebar/interface sizing, placement switches, reduced motion, five optional surface profiles, a constrained desktop/mobile navigation model, and automatic dark-mode settings. Surface profiles merge only validated color and appearance overrides into shared defaults. Navigation destinations and identifiers are fixed; administrators may change order, labels, approved icon classes, visibility, and administrator/technician access, but cannot submit an arbitrary URL or script. Arbitrary CSS and executable values are never accepted. The generated palette is served as `text/css` from the same origin so strict authentication-page Content Security Policy remains intact.

Logo uploads accept PNG, JPEG, WebP, and GIF, while favicon and login-background uploads remain restricted to PNG, JPEG, and WebP. Animated GIF logos are stored without transcoding so their embedded frame timing is preserved; the manager reports frame count and approximate playback rate and flags animations outside its 24fps acceptance window. Static logos can be cropped and resized through bounded pixel inputs when PHP GD is available. PNG/JPEG uploads receive an immutable WebP companion when supported, and cleanup preserves that companion with every referenced revision. Nexus validates the HTTP upload, file size, decoded image type, MIME type, and dimensions; derives the destination extension from inspected content; rejects SVG; and never uses the client filename. Preset/configuration imports apply the same settings validator. Scheduling accepts only a future UTC timestamp and an enable/pause action. Each settings mutation snapshots the previous validated design, and rollback atomically swaps the two designs.

Live surfaces and preview documents resolve settings through the same per-surface function. The technician ticket summary is a shared server-rendered component used by both the live queue and preview. Theme Studio embeds isolated desktop and phone instances side by side, while the existing responsive tester can still inspect arbitrary widths. Automatic dark mode can follow the browser, use a daily overnight schedule, force a mode, or accept a per-user choice stored as a same-site preference; dark colors and light/dark image assets remain independently selectable.

Recovery mode performs bounded health checks for required managed files, readable settings, and balanced CSS structure. Runtime headers use that health gate and silently fall back to native ITFlow presentation if Nexus is paused or unhealthy. Administrators can emergency-disable the presentation marker or restore the newest pinned known-good revision after an automatic safety snapshot; neither action grants the web process shell or package-replacement capability.
...
The command-line bootstrap uses the same lifecycle boundary without exposing privileged web execution. After downloading and checksum-verifying the latest release, `install-latest.sh` resolves the existing root-owned manager state and current package registration. `upgrade.php` invokes both package managers directly without a command shell: the current manager verifies and restores the ITFlow baseline, the replacement manager runs compatibility checks and activation, and the previous package is reinstalled and verified if replacement activation fails. The recorded enabled or disabled mode is preserved.
