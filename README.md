...
Nexus Theme Manager is a lifecycle-managed ITFlow interface package with a full administrator-only Theme Studio, protected root-only installer, and verified GUI update workflow. Administrators can control responsive branding, navigation, login imagery, presets, scheduling, revision history, layout, content, motion, and colors from a private draft workspace without granting the web service permission to execute commands or directly replace application code.
...
![Nexus Theme Studio administration page](docs/images/nexus-theme-studio-v3-5.png)
...
- Nine-part color system with independent sidebar, header, and header-text colors; five curated presets; free-form controls; exact saved-draft previews; and live WCAG contrast feedback.
...
Published settings, private drafts, up to 50 published revisions, schedules, presets, and uploaded branding are stored in ITFlow's writable `uploads` area and survive CLI disable, uninstall, and version upgrades. Immutable branding assets remain available while referenced by the live design, a draft, or revision history; abandoned uploads are cleaned automatically.

## Technician theme preview

![Nexus Theme Manager technician interface](docs/images/nexus-technician-preview.png)

This screenshot was captured from an isolated local ITFlow 26.08/AdminLTE test environment using the release stylesheet and sanitized demonstration data. The Ticket queue row shown in the preview is backed by live, access-scoped ITFlow ticket data in 3.5.0.

## Authenticated client portal preview

![Nexus authenticated client portal navigation](docs/images/nexus-client-portal-v3-1-1.jpg)

The configured portal logo now occupies the persistent navigation brand after sign-in, remains linked to the client home page, and falls back to the configured brand name when portal logo placement is disabled.
...
## Downloadable invoice PDF preview

![Nexus branded downloadable invoice PDF](docs/images/nexus-invoice-pdf-v3-2.png)

Printing keeps a compact, paper-safe Nexus heading. Download uses a dedicated URL-key-validated TCPDF renderer with the active palette, brand, logo, tagline, invoice hierarchy, items, totals, notes, and footer. Pausing Nexus redirects downloads to ITFlow's original PDF renderer; CLI disable or uninstall restores the original invoice link and removes the Nexus PDF files entirely.
...
2. Extract this ZIP outside the ITFlow document root, for example `/opt/Nexus-Theme-Manager-for-ITFlow-3.5.0`.
...
```bash
sudo php /opt/Nexus-Theme-Manager-for-ITFlow-3.5.0/manager.php doctor --root /var/www/itflow.example.com
sudo php /opt/Nexus-Theme-Manager-for-ITFlow-3.5.0/manager.php install --root /var/www/itflow.example.com --yes
```
