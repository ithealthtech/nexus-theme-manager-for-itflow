...
/*
 * Nexus queue pulse. Keep this behind the web presentation switch so pausing
 * the theme removes both the visual row and its aggregate queries immediately.
 * Counts use the same client-access scope as the ticket list, but intentionally
 * ignore the page filters so the cards describe the technician's full queue.
 */
$nexus_ticket_metrics = null;
if (!empty($nexus_theme_enabled)) {
    $nexus_open_scope = "ticket_archived_at IS NULL AND ticket_resolved_at IS NULL AND ticket_closed_at IS NULL $count_where";
    $nexus_metric_sql = mysqli_query($mysqli,
        "SELECT
            COUNT(metric_t.ticket_id) AS open_count,
            SUM(CASE WHEN metric_t.ticket_priority IN ('High', 'Urgent') THEN 1 ELSE 0 END) AS high_priority_count,
            SUM(CASE WHEN COALESCE((
                SELECT metric_reply.ticket_reply_type
                FROM ticket_replies AS metric_reply
                WHERE metric_reply.ticket_reply_ticket_id = metric_t.ticket_id
                AND metric_reply.ticket_reply_archived_at IS NULL
                AND metric_reply.ticket_reply_type IN ('Public', 'Client')
                ORDER BY metric_reply.ticket_reply_id DESC
                LIMIT 1
            ), '') = 'Public' THEN 1 ELSE 0 END) AS waiting_client_count
        FROM tickets AS metric_t
        WHERE $nexus_open_scope"
    );
    $nexus_metric_row = mysqli_fetch_assoc($nexus_metric_sql);

    $nexus_response_where = "ticket_archived_at IS NULL
        AND ticket_first_response_at IS NOT NULL
        AND ticket_first_response_at >= ticket_created_at
        $count_where";
    $nexus_response_count = intval(mysqli_fetch_row(mysqli_query($mysqli,
        "SELECT COUNT(ticket_id) FROM tickets WHERE $nexus_response_where"
    ))[0]);
    $nexus_median_response_seconds = null;

    if ($nexus_response_count > 0) {
        $nexus_response_offset = intdiv($nexus_response_count - 1, 2);
        $nexus_response_limit = $nexus_response_count % 2 === 0 ? 2 : 1;
        $nexus_response_sql = mysqli_query($mysqli,
            "SELECT TIMESTAMPDIFF(SECOND, ticket_created_at, ticket_first_response_at) AS response_seconds
            FROM tickets
            WHERE $nexus_response_where
            ORDER BY response_seconds
            LIMIT $nexus_response_offset, $nexus_response_limit"
        );
        $nexus_response_samples = array();
        while ($nexus_response_row = mysqli_fetch_assoc($nexus_response_sql)) {
            $nexus_response_samples[] = intval($nexus_response_row['response_seconds']);
        }
        if ($nexus_response_samples) {
            $nexus_median_response_seconds = intval(round(array_sum($nexus_response_samples) / count($nexus_response_samples)));
        }
    }

    $nexus_median_response = 'No data';
    if ($nexus_median_response_seconds !== null) {
        if ($nexus_median_response_seconds < 60) {
            $nexus_median_response = '<1m';
        } elseif ($nexus_median_response_seconds < 3600) {
            $nexus_median_response = intval(round($nexus_median_response_seconds / 60)) . 'm';
        } elseif ($nexus_median_response_seconds < 86400) {
            $nexus_response_hours = intdiv($nexus_median_response_seconds, 3600);
            $nexus_response_minutes = intval(round(($nexus_median_response_seconds % 3600) / 60));
            if ($nexus_response_minutes === 60) {
                $nexus_response_hours++;
                $nexus_response_minutes = 0;
            }
            $nexus_median_response = $nexus_response_hours . 'h' . ($nexus_response_minutes ? ' ' . $nexus_response_minutes . 'm' : '');
        } else {
            $nexus_response_days = intdiv($nexus_median_response_seconds, 86400);
            $nexus_response_hours = intval(round(($nexus_median_response_seconds % 86400) / 3600));
            $nexus_median_response = $nexus_response_days . 'd' . ($nexus_response_hours ? ' ' . $nexus_response_hours . 'h' : '');
        }
    }

    $nexus_ticket_metrics = array(
        'open' => intval($nexus_metric_row['open_count'] ?? 0),
        'waiting' => intval($nexus_metric_row['waiting_client_count'] ?? 0),
        'priority' => intval($nexus_metric_row['high_priority_count'] ?? 0),
        'response' => $nexus_median_response,
    );
}
