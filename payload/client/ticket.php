...
if (isset($_GET['id']) && intval($_GET['id'])) {
    ...
    if ($ticket_row) {
        ...
        <div class="card <?= ($nexus_theme_enabled ?? false) ? 'nexus-ticket-thread' : '' ?>">
            <div class="card-header bg-dark text-bg-dark my-2">
        ...
        <?php if (empty($ticket_resolved_at)) { ?>
            ...
            <form action="post.php" enctype="multipart/form-data" method="post" class="<?= ($nexus_theme_enabled ?? false) ? 'card' : '' ?>">
                <?php if ($nexus_theme_enabled ?? false) { ?><div class="card-body"><?php } ?>
                <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">
                <input type="hidden" name="ticket_id" value="<?= $ticket_id ?>">
                <div class="mb-3">
                    <?php if ($nexus_theme_enabled ?? false) { ?><label for="ticket-comment">Reply to this request</label><?php } ?>
                    <textarea class="form-control tinymce" id="ticket-comment" name="comment" placeholder="<?= ($nexus_theme_enabled ?? false) ? 'Add an update' : 'Add comments..' ?>"></textarea>
                </div>
                <div class="mb-3">
                    <?php if ($nexus_theme_enabled ?? false) { ?><label for="fileInput">Attachments</label><?php } ?>
                ...
                <?php if ($nexus_theme_enabled ?? false) { ?>
                    <button type="submit" class="btn btn-primary" name="add_ticket_comment"><i class="fas fa-reply me-2" aria-hidden="true"></i>Send reply</button>
                <?php } else { ?>
                    <button type="submit" class="btn btn-primary" name="add_ticket_comment">Reply</button>
                <?php } ?>
                <?php if ($nexus_theme_enabled ?? false) { ?></div><?php } ?>
            </form>
                    ...
                    <h3 class="card-title">
                        <div class="d-flex">
                            ...
                            if (!empty($user_avatar)) {
                                ?>
                                <img src="<?= $avatar_link ?>" alt="Support conversation participant" class="img-size-50 me-3 rounded-circle">
                                <?php
                            } else {
                                ?>
