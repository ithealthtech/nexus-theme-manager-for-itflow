...
$nexusLogoUrl = nexusThemeLogoUrl($nexusSettings, '', 'light');
$nexusDarkLogoUrl = $nexusSettings['branding']['logo_dark_path'];
$nexusNavigationLogoUrl = nexusThemeLogoUrl($nexusSettings, '', nexusThemeLogoVariantForColor($nexusSettings['colors']['header']));
                        ...
                        <div class="card-body p-0">
                            <div class="nexus-live-preview" id="nexus-live-preview" style="--preview-primary:<?= $nexusSettings['colors']['primary'] ?>;--preview-secondary:<?= $nexusSettings['colors']['secondary'] ?>;--preview-sidebar:<?= $nexusSettings['colors']['sidebar'] ?>;--preview-header:<?= $nexusSettings['colors']['header'] ?>;--preview-header-text:<?= $nexusSettings['colors']['header_text'] ?>;--preview-auth:<?= $nexusSettings['colors']['auth_background'] ?>;--preview-page:<?= $nexusSettings['colors']['page'] ?>;--preview-surface:<?= $nexusSettings['colors']['surface'] ?>;--preview-text:<?= $nexusSettings['colors']['text'] ?>;--preview-overlay:<?= $nexusSettings['branding']['login_background_overlay'] / 100 ?>;--preview-bg-position:<?= $nexusSettings['branding']['login_background_position'] ?> center;<?php if ($nexusSettings['branding']['login_background_path'] !== '') { ?>--preview-bg-image:url('<?= escapeHtml($nexusSettings['branding']['login_background_path']) ?>');<?php } ?>">
                                ...
                                <div class="nexus-preview-shell nexus-preview-panel d-none" data-preview-panel="navigation" data-width="<?= (int)$nexusSettings['appearance']['sidebar_width'] ?>" data-density="<?= escapeHtml($nexusSettings['appearance']['menu_density']) ?>" data-header-style="<?= escapeHtml($nexusSettings['appearance']['header_style']) ?>" data-navigation-style="<?= escapeHtml($nexusSettings['appearance']['navigation_style']) ?>">
                                    ...
                                    <div class="nexus-preview-main"><header><i class="fas fa-bars"></i><span></span><i class="fas fa-bell"></i><i class="fas fa-user-circle"></i></header><main><div class="nexus-preview-page-heading"></div><div class="nexus-preview-stat-grid"><i></i><i></i><i></i></div><div class="nexus-preview-content-block"></div></main></div>
                                </div>
                            </div>
                        </div>
                    </section>
...
(function () {
    ...
    studio.querySelectorAll('.nexus-preview-mode').forEach(function (button) { button.addEventListener('click', function () { studio.querySelectorAll('.nexus-preview-mode').forEach(function (item) { item.classList.toggle('active', item === button); item.classList.toggle('btn-info', item === button); item.classList.toggle('btn-outline-info', item !== button); }); studio.querySelectorAll('.nexus-preview-panel').forEach(function (panel) { panel.classList.toggle('d-none', panel.dataset.previewPanel !== button.dataset.mode); }); document.getElementById('nexus-preview-title').textContent = button.dataset.mode === 'navigation' ? 'Sidebar & header' : 'Authentication'; }); });
    function updateNavigationPreview() { var shell = studio.querySelector('.nexus-preview-shell'); if (!shell) return; var width = document.getElementById('nexus-sidebar-width').value; shell.dataset.width = width; shell.style.setProperty('--preview-sidebar-width', Math.max(31, Math.min(46, width / 7)) + '%'); shell.dataset.density = document.getElementById('nexus-menu-density').value; shell.dataset.headerStyle = document.getElementById('nexus-header-style').value; shell.dataset.navigationStyle = document.getElementById('nexus-navigation-style').value; shell.classList.toggle('is-compact', document.getElementById('nexus-sidebar-compact').checked); document.getElementById('nexus-sidebar-width-output').textContent = width + 'px'; }
    studio.querySelectorAll('.nexus-navigation-control').forEach(function (control) { control.addEventListener('input', updateNavigationPreview); control.addEventListener('change', updateNavigationPreview); });
