...
foreach (['auth', 'technician', 'client', 'invoice'] as $nexusPreviewSurface) {
            ...
            <div class="col-xl-5 mt-4 mt-xl-0 nexus-workspace-side-column">
                <div class="nexus-preview-sticky">
                    <section class="card nexus-preview-card nexus-workspace-preview">
                        ...
                        <div class="card-footer bg-transparent"><small class="text-muted" id="nexus-preview-freshness"><i class="fas fa-info-circle mr-1"></i>Editing the form does not alter this trustworthy snapshot. Save draft to regenerate it.</small></div>
                    </section>
                    ...
                    <section class="card nexus-design-operations nexus-workspace-panel" data-workspace-panel="operations" id="nexus-operations">
                        ...
                        <div class="card-body pt-0">
                            ...
                            <section class="nexus-revision-history mb-4">
                                ...
                                <?php if ($nexusRevisions !== []) { ?><div class="nexus-revision-list"><?php foreach (array_slice($nexusRevisions, 0, 12) as $nexusRevision) { $nexusRevisionIsActive = $nexusRevision['hash'] === $nexusActiveHash; $nexusRevisionDiff = nexusThemeSettingsDiff($nexusActiveSettings, $nexusRevision['settings']); $nexusRevisionChanges = count($nexusRevisionDiff); ?><article><div><strong><?= escapeHtml($nexusRevision['action']) ?><?php if ($nexusRevisionIsActive) { ?> <span class="badge badge-success">Live</span><?php } ?></strong><small><?= escapeHtml($nexusRevision['actor']) ?> · <?= escapeHtml($nexusRevision['created_at']) ?> · <?= $nexusRevisionChanges ?> difference<?= $nexusRevisionChanges === 1 ? '' : 's' ?></small><code><?= escapeHtml(substr($nexusRevision['hash'], 0, 10)) ?></code><?php if ($nexusRevisionDiff !== []) { ?><details class="nexus-revision-diff"><summary>Compare with live</summary><ul><?php foreach ($nexusRevisionDiff as $nexusRevisionChange) { ?><li><strong><?= escapeHtml(str_replace('_', ' ', $nexusRevisionChange['path'])) ?></strong><span><?= escapeHtml($nexusRevisionChange['before'] === '' ? '—' : $nexusRevisionChange['before']) ?> → <?= escapeHtml($nexusRevisionChange['after'] === '' ? '—' : $nexusRevisionChange['after']) ?></span></li><?php } ?></ul></details><?php } ?></div><button type="submit" form="nexus-restore-revision-<?= escapeHtml($nexusRevision['id']) ?>" class="btn btn-sm btn-outline-info" <?= $nexusRevisionIsActive ? 'disabled' : '' ?> onclick="return confirm('Load this published revision into the private draft workspace?')">Restore to draft</button></article><?php } ?></div><?php } else { ?><div class="nexus-empty-state"><i class="fas fa-history"></i><span>The current design and every new publication will appear here after the first publish.</span></div><?php } ?>
                            </section>
                    ...
                    <section class="card nexus-workspace-panel" data-workspace-panel="system">
                        ...
                        <div class="card-body pt-0"><p class="text-muted">Move designs between Nexus installations or load the factory palette into the private draft.</p><div class="btn-group w-100"><button type="button" class="btn btn-outline-info" id="nexus-export"><i class="fas fa-download mr-1"></i>Export current workspace</button><button type="button" class="btn btn-outline-info" data-toggle="modal" data-target="#nexus-import-modal"><i class="fas fa-upload mr-1"></i>Import to draft</button><button type="submit" form="nexus-reset-form" class="btn btn-outline-danger" <?= $nexusControlWritable ? '' : 'disabled' ?> onclick="return confirm('Load every Nexus default into the private draft? The live design will not change.')"><i class="fas fa-undo mr-1"></i>Defaults to draft</button></div><textarea class="d-none" id="nexus-export-json"><?= escapeHtml((string)$nexusExportJson) ?></textarea></div>
                    </section>
                </div>
            </div>
                </div>
